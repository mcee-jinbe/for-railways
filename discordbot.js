...
function birthday_check() {
  ...
  profileModel.findOne({ birthday_month: today_month, birthday_day: today_day }, function(err, model) {
    if (err) {
    ...
    if (model == null) {
      console.log("今日(" + today_month + "月" + today_day + "日)、誕生日の人は確認できませんでした。")
      return;
    } else {
      // めでたい人の情報を取得して定義
      ...
      if (birthday_status == "finished") {
        ...
        client.channels.cache
          ...
          .send({
            ...
            files: [{
              attachment: './photos/jinbe_ome.png', name: 'happy_birthday.png'
            }],
          });
